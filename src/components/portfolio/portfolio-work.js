  ...
  return (
    <section className="pb-20 pt-3 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="grid md:grid-cols-3 space-y-6 gap-6">
          {workCategories.map((category, index) => (
            ...
            >
              <div className="relative aspect-[9/11] rounded overflow-hidden mb-6 shadow-lg">
                <Image src={category.image || "/placeholder.svg"} alt={category.title} fill className="object-cover" />
              </div>

              <h2 className="font-serif text-xl mb-2 text-[#2C2C2C]">{category.title}</h2>

              <Link
                href={category.link}
                className="inline-block bg-[#5A6F5F] text-white px-8 py-3 rounded-full text-xs font-semibold hover:bg-[#4A5F4F] transition-colors"
              >
                SEE EXAMPLES
              </Link>
            </motion.div>
